class AgentNotifications::ConversationNotificationsMailer < ApplicationMailer
  def conversation_creation(conversation, agent, _user)
    ...
    subject = "#{@agent.available_name}, nueva conversación [#{@conversation.display_id}] creada en #{inbox_name}."
  ...
  def conversation_assignment(conversation, agent, _user)
    ...
    @agent = agent
    @conversation = conversation
    subject = "#{@agent.available_name}, tienes una nueva conversación asignada [#{@conversation.display_id}]."
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
  ...
  def conversation_mention(conversation, agent, message)
    ...
    @conversation = conversation
    @message = message
    subject = "#{@agent.available_name}, te han mencionado en la conversación [#{@conversation.display_id}]."
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
  ...
  def assigned_conversation_new_message(conversation, agent, message)
    ...
    @agent = agent
    @conversation = conversation
    subject = "#{@agent.available_name}, nuevo mensaje en tu conversación asignada [#{@conversation.display_id}]."
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
  ...
  def participating_conversation_new_message(conversation, agent, message)
    ...
    @agent = agent
    @conversation = conversation
    subject = "#{@agent.available_name}, nuevo mensaje en la conversación [#{@conversation.display_id}]."
    @action_url = app_account_conversation_url(account_id: @conversation.account_id, id: @conversation.display_id)
    send_mail_with_liquid(to: @agent.email, subject: subject) and return
  end
